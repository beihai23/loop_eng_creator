...
func (s *Store) GetTask(id string) (TaskRow, error) {
	row := s.db.QueryRow(
		`SELECT id, issue_ref, description, task_type, source, acceptance_criteria_json, COALESCE(body,''), COALESCE(agent,''), COALESCE(title,'') FROM tasks WHERE id=?`, id)
	var t TaskRow
	var critJSON string
	if err := row.Scan(&t.ID, &t.IssueRef, &t.Description, &t.TaskType, &t.Source, &critJSON, &t.Body, &t.Agent, &t.Title); err != nil {
		return t, err
	}
...
func (s *Store) TaskSpecsByRef() (map[string]TaskRow, error) {
	rows, err := s.db.Query(
		`SELECT id, issue_ref, description, acceptance_criteria_json, updated_at, COALESCE(body,''), COALESCE(agent,''), COALESCE(title,'') FROM tasks`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var t TaskRow
		var critJSON string
		if err := rows.Scan(&t.ID, &t.IssueRef, &t.Description, &critJSON, &t.UpdatedAt, &t.Body, &t.Agent, &t.Title); err != nil {
			return nil, err
		}
		_ = json.Unmarshal([]byte(critJSON), &t.Criteria)
		out[t.IssueRef] = t
...
func (s *Store) NextReadyTask() (TaskRow, bool, error) {
	row := s.db.QueryRow(
		`SELECT t.id, t.issue_ref, t.description, t.task_type, t.source, t.acceptance_criteria_json, COALESCE(t.body,''), COALESCE(t.agent,''), COALESCE(t.title,'')
		 FROM tasks t
...
func scanTaskRows(rows *sql.Rows) ([]TaskRow, error) {
	...
	for rows.Next() {
		var t TaskRow
		var critJSON string
		if err := rows.Scan(&t.ID, &t.IssueRef, &t.Description, &t.TaskType, &t.Source, &critJSON, &t.Body, &t.Agent, &t.Title); err != nil {
			return nil, err
		}
		_ = json.Unmarshal([]byte(critJSON), &t.Criteria)
		out = append(out, t)
