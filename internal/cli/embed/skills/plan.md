...
{{if .HumanFeedback}}
## 人反馈（issue 上人的评论——务必据此规划，不要无视）
{{.HumanFeedback}}
{{end}}
{{if .RunHistory}}
## 本任务历轮 run 摘要（来自状态库的结构化记录：每行 = 一轮已终结 run 的结局 + 最近驳回理由）

下面是本 issue 之前各轮 run 的真实结局。它替代了过去「读回战报评论」的角色——
issue 里的战报是给人看的写回，不再喂给你；这份摘要才是你的跨轮记忆。据此判断：
同一条驳回反复出现时，不要再沿旧路重掷，换思路或修订合同。
...
{{end}}
{{if .RejectedDiff}}
## 上一轮被驳回的实现（现场 diff——重试诊断/历轮摘要是「判决」，这是「现场」）

下面是上一轮 execute 实际写出、但被 verify 驳回的完整改动。驳回理由见上面的
重试诊断/历轮 run 摘要。**在它的基础上修正，还是判断方向错误后推倒重来，由你带着完整
...
> 只读**，不是靠你自觉。读文件 / grep / 跑只读命令（`git ls-files`、`go doc` 等）照常
> 可用，探索能力不受影响。prompt 层的「只读不写」与权限层的工具移除是**双保险**——
> 你照常只读探索、只产出计划 JSON 即可。
